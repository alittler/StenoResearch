  ...
  return (
    <div className="space-y-8 animate-fade-in">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="p-6 bg-slate-50 rounded-2xl border border-slate-200 flex flex-col justify-between">
          <p className="text-[10px] font-bold font-mono text-slate-500 uppercase">Concepts</p>
          <h2 className="text-4xl font-black font-mono text-slate-900">{imageCount}</h2>
          <button onClick={() => onNavigate('visualizer')} className="text-[9px] font-black font-mono text-blue-600 uppercase mt-4 hover:underline text-left">Moodboard</button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <section className="space-y-4">
          <h3 className="text-sm font-black font-mono text-slate-400 uppercase tracking-widest">Intelligence Synthesis</h3>
          <div className="grid grid-cols-2 gap-4">
            <button onClick={() => onNavigate('brief')} className="p-8 bg-slate-900 border border-slate-800 rounded-2xl hover:bg-black transition-all text-center group shadow-xl">
              <span className="text-3xl block mb-2 group-hover:scale-110 transition-transform">📑</span>
              <span className="text-[10px] font-black font-mono text-white uppercase tracking-widest">Project Brief</span>
            </button>
            <button onClick={() => onNavigate('visualizer')} className="p-8 bg-stone-100 border border-stone-200 rounded-2xl hover:bg-white transition-all text-center group shadow-xl">
              <span className="text-3xl block mb-2 group-hover:scale-110 transition-transform">🎨</span>
              <span className="text-[10px] font-black font-mono text-stone-900 uppercase tracking-widest">Visualizer</span>
            </button>
          </div>
        </section>
      </div>
